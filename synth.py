...
class Wavesynth:
    ...
    variable harmonics, white noise.
    """
    ...
    def sine(self, frequency, duration, amplitude=1.0, phase=0.0, fmlfo=None):
        ...
        # @TODO apply fm LFO  y=sin(t+t*LFO)
        samples = self._get_array()
        scale = 2**(self.samplewidth*8-1)-1
        waveform = self.oscillator.sine(frequency, amplitude, phase)
        for _ in range(int(duration*self.samplerate)):
            samples.append(int(next(waveform)*scale))
        return samples

    def square(self, frequency, duration, amplitude=0.8, phase=0.0):
        """
        ...
        samples = self._get_array()
        scale = 2**(self.samplewidth*8-1)-1
        waveform = self.oscillator.square(frequency, amplitude, phase)
        for _ in range(int(duration*self.samplerate)):
            samples.append(int(next(waveform)*scale))
        return samples

    def square_h(self, frequency, duration, num_harmonics=12, amplitude=1.0, phase=0.0):
    ...
    def triangle(self, frequency, duration, amplitude=1.0, phase=0.0):
        ...
        samples = self._get_array()
        scale = 2**(self.samplewidth*8-1)-1
        waveform = self.oscillator.triangle(frequency, amplitude, phase)
        for _ in range(int(duration*self.samplerate)):
            samples.append(int(next(waveform)*scale))
        return samples

    def sawtooth(self, frequency, duration, amplitude=0.8, phase=0.0):
        """Perfect sawtooth waveform (not using harmonics)."""
        assert 0 <= amplitude <= 1.0
        samples = self._get_array()
        scale = 2**(self.samplewidth*8-1)-1
        waveform = self.oscillator.sawtooth(frequency, amplitude, phase)
        for _ in range(int(duration*self.samplerate)):
            samples.append(int(next(waveform)*scale))
        return samples

    def sawtooth_h(self, frequency, duration, num_harmonics=12, amplitude=0.8, phase=0.0):
        """Sawtooth waveform based on harmonic sine waves"""
        assert 0 <= amplitude <= 1.0
        samples = self._get_array()
        scale = 2**(self.samplewidth*8-1)-1
        waveform = self.oscillator.sawtooth_h(frequency, num_harmonics, amplitude, phase)
        for _ in range(int(duration*self.samplerate)):
            samples.append(int(next(waveform)*scale))
        return samples

    def pulse(self, frequency, width, duration, amplitude=0.8, phase=0.0):
        """Perfect pulse waveform (not using harmonics)."""
        assert 0 <= amplitude <= 1.0
        assert 0 < width <= 0.5
        samples = self._get_array()
        scale = 2**(self.samplewidth*8-1)-1
        waveform = self.oscillator.pulse(frequency, width, amplitude, phase)
        for _ in range(int(duration*self.samplerate)):
            samples.append(int(next(waveform)*scale))
        return samples
    ...
    def harmonics(self, frequency, duration, num_harmonics, amplitude=1.0, phase=0.0, only_even=False, only_odd=False):
        ...
        samples = self._get_array()
        scale = 2**(self.samplewidth*8-1)-1
        waveform = self.oscillator.harmonics(frequency, num_harmonics, amplitude, phase, only_even=only_even, only_odd=only_odd)
        for _ in range(int(duration*self.samplerate)):
            samples.append(int(next(waveform)*scale))
        return samples
